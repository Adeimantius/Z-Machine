...
namespace zmachine
{
    partial class Machine { 
        ...
        public void pushRoutineData(List<ushort> operands)
        {
            ...
            if (callDepth >= 128)
            {
                Debug.LogWarning("Error: Call Stack Overflow"); //alert the user 
                finish = true;
                return;
            }
...
//            Next, the arguments are written into the local variables (argument 1 into local 1 and so on).
        ...
        public void popRoutineData(ushort returnVal)
        {
            if (callDepth == 0)
            {
                ...
                Debug.LogWarning("Error: Call Stack Underrun"); // alert the user to the error.
                finish = true;
            }
...
//            Debug.Log(pcStart.ToString("X4") + " : 0x" + instruction.ToString("X2") + " (" + instruction + ")");
            ...
            case 0x01:
                {
//                    Debug.Log("\tInstruction Form : Long");
            ...
            case 0x02:                              // Define short-form instruction
                {
//                    Debug.Log("\tInstruction Form : Short");
            ...
            case 0x03:                             // Define Variable form instruction
                    {
//                        Debug.Log("\tInstruction Form : Variable");
...
//           CCDD0000                                           | 
        ...
        void process2OP(int opcode, List<ushort> operands) 
        {
            ...
            if (debug)
            {
                Debug.Log(pcStart.ToString("X4") + "  " + stateString() + " : [2OP/" + opcode.ToString("X2") + "] " + op.name());
        ...
        void process1OP(int opcode, ushort operand1) 
        {
            ...
            if (debug)
                Debug.Log(pcStart.ToString("X4") + "  " + stateString() + " : [1OP/" + opcode.ToString("X2") + "] " + op.name() + " " + operand1.ToString());
        ...
        void process0OP(int opcode)
        {
            ...
            if (debug)
                Debug.Log(pcStart.ToString("X4") + "  " + stateString() + " : [0OP/" + opcode.ToString("X2") + "] " + op.name());
            op.run(this);
        }
        void processVAR(int opcode, List<ushort> operands) 
        {
            ...
            if (debug)
            {
                Debug.Log(pcStart.ToString("X4") + "  " + stateString() + " : [VAR/" + opcode.ToString("X2") + "] " + op.name());
                foreach (ushort v in operands)
                    Debug.Log(" " + v);
                Debug.Log("");
            }
...
// --------------------------------------------------------------------------------------------
        ...
        public void branch(bool condition)        // Check branch instructions
         {
//             Debug.Log("BRANCH: " + (condition ? "1" : "0"));
