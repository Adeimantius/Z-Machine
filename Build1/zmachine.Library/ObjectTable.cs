...
public class ObjectTable
{
    ...
    private int tp; // pointer to move through tables
    ...
    public int
        getDefaultProperty(int property) // Start reading from the Program Defaults Table (before list of objects)
    {
        this.tp = this.Machine.Memory.getWord(Memory.ADDR_OBJECTS);
    ...
    {
        this.tp = this.Machine.Memory.getWord(Memory.ADDR_OBJECTS) + 31 * 2;
        return this.tp;
    }

    public int
        getPropertyTableAddress(
            int objectId) // Return property table address for given object. Leaves Table Pointer at beginning of next property.
    {
        this.getObjectTable();
    ...
    public int
        getObjectPropertyAddress(int objectId,
            int property) // Get property address for an object's property (if it exists)
    {
        int propertyAddress;
        this.tp = this.getPropertyTableAddress(objectId);
        int
            text_length =
                this.tp_getByte(); // Read the first byte of the property address, figure out the text-length, then skip forward to the first property address.
        this.tp += text_length * 2; // skip past header

        int sizeByte = this.tp_getByte(); // get initial sizeByte (property 1)
        while
            (sizeByte > 0) // Let the search begin! Move through properties looking for a propertyId matching the property given.
        {
    ...
    public int
        getNextObjectPropertyIdAfter(int objectId,
            int property) // Get property address down the chain given a certain object.
    {
        int propLen;
        int nextPropertyId;
        byte sizeByte;
        ...
        sizeByte = this.Machine.Memory.getByte((uint)this.tp - 1); // Find length of property
    ...
    public int getObjectPropertyLengthFromAddress(int propertyAddress)
    {
        ...
        int sizeByte =
            this.Machine.Memory.getByte((uint)propertyAddress -
                                        1); // arranged as 32 times the number of data bytes minus one, plus the property number
        int propLen = sizeByte / 32 + 1;
        return propLen;
    }
    ...
    public bool getObjectAttribute(int objectId, int attributeId)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                attributes[i * 8 + j] =
                    ((attributeByte >> (7 - j)) & 0x01) >
                    0; // Take the next byte and cut the first (MSB) value off the top. Add to boolean array attribute[] and voila!
            }
        }
    ...
    public ObjectTable setObjectAttribute(int objectId, int attributeId, bool value)
    {
        ...
        if (value)
        {
            a = this.Machine.Memory.getByte(address +
                                            attributeByte); // Read the byte given by the attribute segment our attribute Id is in.
            a |= (byte)(1 << attributeShift); // Fill a bit at the given shift in that byte.
        }
        else
        {
            a = this.Machine.Memory.getByte(address + attributeByte);
        ...
        this.Machine.Memory.setByte(address + attributeByte, a);
        return this;
    }
    ...
    public ObjectTable setObjectProperty(int objectId, int property, ushort value)
    {
        ...
        {
            this.Machine.Memory.setWord((uint)propertyAddress, (ushort)(value & 0xff));
        }
        else if (propLen == 2)
        {
            this.Machine.Memory.setWord((uint)propertyAddress, value);
        }
    ...
    {
        this.tp = this.getObjectAddress(objectId) + 4; // move past object header (4 attribute bytes)
        this.Machine.Memory.setByte((uint)this.tp, (byte)parentId);
        return this;
    }
    ...
    public ObjectTable setSibling(int objectId, int siblingId)
    {
        ...
        this.Machine.Memory.setByte((uint)this.tp, (byte)siblingId);
        return this;
    }
    ...
    {
        this.tp = this.getObjectAddress(objectId) + 6; // move past object header (4 attribute bytes + parent byte + sibling byte)
        this.Machine.Memory.setByte((uint)this.tp, (byte)childId);
        return this;
    }
    ...
    {
        byte next = this.Machine.Memory.getByte((uint)this.tp);
        this.tp++;
        return next;
    }
    ...
    {
        ushort next = this.Machine.Memory.getWord((uint)this.tp);
        this.tp += 2;
        return next;
    }
    ...
    public string objectName(int objectId)
    {
        ...
        name = this.Machine.Memory.getZSCII((uint)this.tp, (uint)textLength * 2).str;
        return name;
    }
